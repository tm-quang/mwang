...
// =================================================================================
const API_URL = "https://script.google.com/macros/s/AKfycbxQO9tJOT4qkWHbumWZHAXiZCTaadUTQEusEPeFt7oLav5oEsoSQJqrPac-Fznha71i/exec";
const leftMenuData = [{
    title: 'ADMIN',
    items: [{
...
let searchableMenuItems = [], dropdownTimeout, isAdminAuthenticated = !1, countdownInterval, isSidebarPinned = !1, isMobileView = 1080 >= window.innerWidth, errorAlertTimeout, successAlertTimeout;
...
function showLoadingIndicator() { const e = document.getElementById("loadingIndicator-login"); e && (e.style.display = "flex") }
function hideLoadingIndicator() { const e = document.getElementById("loadingIndicator-login"); e && (e.style.display = "none") }
function clearForm(e) { const t = document.getElementById(e); t && t.reset() }
...
function hideAllDropdowns() { document.querySelectorAll(".dropdown-menu.show").forEach(e => { e.classList.remove("show"), e.previousElementSibling?.classList.remove("active") }) }
function collapseSidebar(e) { isMobileView || e.classList.add("collapsed"), hideAllDropdowns() }
function expandSidebar(e) { isMobileView || e.classList.remove("collapsed") }
function renderLeftMenu() { const e = document.querySelector("#left-sidebar-container .sidebar-content-wrapper"); if (!e) return; e.innerHTML = "", leftMenuData.forEach(t => { const o = document.createElement("div"); o.className = "menu-section", o.innerHTML = `<h3 class="menu-section-title"><span>${t.title}</span></h3>`; const n = document.createElement("div"); n.className = "menu-items-container", t.items.forEach(t => { if (t.isDropdown) { const i = document.createElement("div"); i.className = "dropdown"; const s = document.createElement("div"); s.className = `dropdown-button ${t.className || ""}`, s.innerHTML = `<i class="${t.icon} icon"></i><span>${t.text}</span>`, s.id = t.id; const a = document.createElement("div"); a.className = "dropdown-menu", t.subItems.forEach(e => { const t = document.createElement("a"); t.href = "#", t.id = e.id, t.className = `menu-button-sidebar ${e.className || ""}`, t.innerHTML = `<i class="${e.icon} icon"></i><span>${e.text}</span>`, t.addEventListener("click", t => { t.preventDefault(), hideAllDropdowns(), loadFunctionContent(e), isSidebarPinned || !isMobileView || closeMobileSidebar() }), a.appendChild(t) }), i.appendChild(s), i.appendChild(a), n.appendChild(i); const d = () => { hideAllDropdowns(); const e = s.getBoundingClientRect(); a.style.top = e.top + "px", a.style.left = e.right + 10 + "px", a.classList.add("show"), s.classList.add("active") }; t.isAdmin ? (s.style.cursor = "pointer", s.addEventListener("click", () => { "guest" !== sessionStorage.getItem("userMode") && (isAdminAuthenticated ? d() : (hideAllDropdowns(), document.getElementById("adminLoginError").textContent = "", document.getElementById("adminLoginModal").style.display = "flex", document.getElementById("adminUsername").focus())) }), i.onmouseleave = () => { dropdownTimeout = setTimeout(() => { a.classList.remove("show"), s.classList.remove("active") }, 300) }, a.onmouseenter = () => clearTimeout(dropdownTimeout)) : (() => { a.classList.remove("show"), s.classList.remove("active") }, s.onmouseenter = () => { isMobileView || (clearTimeout(dropdownTimeout), d()) }, s.onclick = () => { isMobileView && d() }, s.onmouseleave = () => { isMobileView || (dropdownTimeout = setTimeout(hideMenu, 300)) }, a.onmouseenter = () => clearTimeout(dropdownTimeout), a.onmouseleave = () => { isMobileView || (dropdownTimeout = setTimeout(hideMenu, 300)) }) } else { const e = document.createElement("a"); e.href = "#", e.id = t.id, e.className = `menu-button-sidebar ${t.className || ""}`, e.innerHTML = `<i class="${t.icon} icon"></i><span>${t.text}</span>`, e.addEventListener("click", e => { e.preventDefault(), hideAllDropdowns(), loadFunctionContent(t), isSidebarPinned || !isMobileView || closeMobileSidebar() }), n.appendChild(e) } }), o.appendChild(n), e.appendChild(o) }) }
function renderRightMenu() { const e = document.querySelector("#right-sidebar-container .sidebar-content-wrapper"); if (!e) return; e.innerHTML = "", rightMenuData.forEach(t => { const o = document.createElement("h3"); o.innerHTML = `<i class="${t.icon}"></i><span>${t.title}</span>`; const n = document.createElement("div"); n.className = "right-menu-section", t.items.forEach(e => { const t = document.createElement("a"); t.href = e.href, t.target = "_blank", t.className = `link-button-right ${e.className || ""}`, t.innerHTML = `<i class="${e.icon} icon"></i><span>${e.text}</span>`, n.appendChild(t) }), e.appendChild(o), e.appendChild(n) }) }
async function loadFunctionContent(e) { if (!e || "object" != typeof e || !("id" in e)) return void console.error("loadFunctionContent được gọi với đối số không hợp lệ:", e); const t = document.getElementById("functionContent"), o = document.getElementById("loadingSpinner"), n = document.getElementById("current-page-title"); t.innerHTML = "", o.style.display = "block", n.textContent = e.pageTitle || e.text || ""; try { let n = ""; if (e.htmlFile) { const t = await fetch(e.htmlFile); if (!t.ok) throw new Error(`Không thể tải file ${e.htmlFile}: ${t.statusText}`); n = await t.text() } else { if (!e.functionName) throw new Error("Cấu hình menu không hợp lệ (thiếu htmlFile hoặc functionName)."); { const t = await callApi("getPageContent", { functionName: e.functionName }); if (!t.success || !t.html) throw new Error(t.message || "Không nhận được nội dung hợp lệ từ API."); n = t.html } } o.style.display = "none", t.innerHTML = n, "btnTimSieuThi" === e.id ? initSearchStorePage() : "btnTimHangBK" === e.id && initSearchHangBKPage(), Array.from(t.querySelectorAll("script")).forEach(e => { const t = document.createElement("script"); Array.from(e.attributes).forEach(e => t.setAttribute(e.name, e.value)), t.appendChild(document.createTextNode(e.innerHTML)), e.parentNode.replaceChild(t, e) }), closeMobileSidebar() } catch (e) { o.style.display = "none", t.innerHTML = `<p style="color: red; text-align: center;">Lỗi tải nội dung: ${e.message}</p>` } }
function setupCollapseListeners() { document.getElementById("functionContent").addEventListener("click", function (e) { const t = e.target.closest(".notification-header-pb3"); if (!t) return; const o = t.closest(".notification-card-pb3"); !o || e.target.closest("a") || (o.classList.contains("collapsed") && document.querySelectorAll(".notification-card-pb3").forEach(e => { e !== o && e.classList.add("collapsed") }), o.classList.toggle("collapsed")) }) }
async function loadNotificationsPage() { const e = document.getElementById("functionContent"), t = document.getElementById("loadingSpinner"), o = document.getElementById("current-page-title"); e.innerHTML = "", t.style.display = "block", o.textContent = "Trang Chủ"; try { const t = await callApi("getNotifications"); if (!t.success || !Array.isArray(t.data)) throw new Error(t.message || "Dữ liệu trả về không hợp lệ."); renderNotifications(t.data) } catch (t) { e.innerHTML = `<p style="color: red; text-align: center;">Lỗi tải thông báo: ${t.message}</p>` } finally { t.style.display = "none" } }
function renderNotifications(e) { const t = document.getElementById("functionContent"), o = '<div class="running-text-marquee"><p>Trang đang được hoàn thiện, nếu có lỗi trong quá trình sử dụng | Liên hệ để báo lỗi hoặc góp ý: 112080 - Trần Minh Quang | ĐT: 039 418 1140 | Cảm ơn!</p></div>'; if (1 === e.length && "Lỗi" === e[0].category) return void (t.innerHTML = `<p style="color: red; text-align: center;">${e[0].message}</p>`); const n = e.filter(e => "Triển khai" === e.category), i = e.filter(e => "Nội bộ" === e.category), s = (e, t, o) => { let n = `<div class="content-column"><h2 class="column-title"><i class="fas ${t}"></i> ${e}</h2><div class="notification-list">`; if (o.length > 0) o.forEach(e => { const t = e.isNew ? '<span class="new-badge">MỚI</span>' : "", o = e.type ? `<span class="type-badge type-${e.type.toLowerCase().replace(/[\/\\s&]/g, "-")}">${e.type}</span>` : "", i = e.link ? `<a href="${e.link}" target="_blank" class="notification-link-btn-pb3"><i class="fas fa-link"></i> Link chi tiết</a>` : "", s = e.updateDate ? `<span class="update-date-badge"><i class="fas fa-calendar-check"></i> ${e.updateDate}</span>` : "", a = e.deadline ? `<span class="deadline-badge"><i class="fas fa-hourglass-half"></i> Deadline: ${e.deadline}</span>` : ""; n += `<div class="notification-card-pb3 collapsed"><div class="notification-header-pb3"><i class="${e.icon} icon"></i><h4>${e.title}</h4>${o}${t}<i class="fas fa-chevron-down expand-icon"></i></div><div class="notification-message-pb3">${e.message}<div class="notification-footer-pb3"><div class="footer-left">${s}</div><div class="footer-center">${a}</div><div class="footer-right">${i}</div></div></div></div>` }); else n += "<p>Không có thông báo nào.</p>"; return n += "</div></div>" }; t.innerHTML = o + '<div class="columns-container-pb2">' + s("THÔNG BÁO CÔNG VIỆC TRIỂN KHAI", "fa-bullhorn", n) + s("THÔNG BÁO CÔNG VIỆC MTAY2", "fa-users", i) + "</div>", setupCollapseListeners() }
function goToHomePage() { loadNotificationsPage() }
function initSearchStorePage() { const e = document.getElementById("maSTInput"), t = document.getElementById("searchButton"), o = document.getElementById("clearButton"); e && t && o && (e.addEventListener("input", handleStoreSuggestionInput), e.addEventListener("keypress", e => { "Enter" === e.key && (e.preventDefault(), handleSearchStore()) }), t.addEventListener("click", handleSearchStore), o.addEventListener("click", clearStoreSearch)) }
async function handleStoreSuggestionInput(e) { const t = e.target, o = document.getElementById("suggestions-box"), n = t.value; if (!o) return; if (n.length < 2) return o.style.display = "none", void (o.innerHTML = ""); try { const e = await callApi("getStoreSuggestions", { partialCode: n }); e && e.length > 0 ? (o.innerHTML = "", e.forEach(e => { const n = document.createElement("div"); n.className = "suggestion-item", n.innerHTML = `<span class="code">${e.code}</span><span class="name">${e.name}</span>`, n.onclick = () => { t.value = e.code, o.style.display = "none", handleSearchStore() }, o.appendChild(n) }), o.style.display = "block") : o.style.display = "none" } catch (e) { console.error("Lỗi khi lấy gợi ý:", e), o.style.display = "none" } }
async function handleSearchStore() { const e = document.getElementById("maSTInput"), t = document.getElementById("resultOutput"), o = document.getElementById("errorMessage"), n = document.getElementById("loadingMessage"), i = document.getElementById("searchButton"), s = document.getElementById("buttonText"), a = document.getElementById("suggestions-box"), d = e.value; a.style.display = "none", t.style.display = "none", o.textContent = "", e.classList.remove("error"); if (!d.trim()) return o.textContent = "Vui lòng nhập Mã Siêu Thị để tìm kiếm.", void e.classList.add("error"); i.disabled = !0, s.textContent = "Đang tìm...", n.style.display = "block"; try { const n = await callApi("searchStore", { maST: d }); n && n.error ? (o.textContent = n.message, e.classList.add("error")) : n ? (t.innerHTML = formatStoreSearchResult(n), t.style.display = "block") : (o.textContent = `Không tìm thấy thông tin cho Mã Siêu Thị: "${d}".`, e.classList.add("error")) } catch (t) { o.textContent = "Lỗi kết nối máy chủ: " + t.message, e.classList.add("error") } finally { i.disabled = !1, s.textContent = "Tìm Kiếm", n.style.display = "none" } }
function clearStoreSearch() { const e = document.getElementById("maSTInput"), t = document.getElementById("resultOutput"), o = document.getElementById("errorMessage"), n = document.getElementById("suggestions-box"); e.value = "", t.style.display = "none", o.textContent = "", e.classList.remove("error"), n && (n.style.display = "none") }
function formatStoreSearchResult(e) { const t = (e, t, o, n) => `<div class="result-row" style="animation-delay: ${n}s;"><i class="fas ${e} result-icon"></i><span class="result-label">${t}</span><span class="result-value">${o || "N/A"}</span></div>`; return `<div class="result-card"><div class="result-main-title">KẾT QUẢ TÌM KIẾM: ${e.maCN}</div><div class="result-section"><div class="result-section-title"><i class="fas fa-info-circle"></i> THÔNG TIN SIÊU THỊ</div>${t("fa-barcode", "Mã CN:", `<strong>${e.maCN}</strong>`, .1)}${t("fa-store", "Tên ST:", `<strong>${e.tenST}</strong>`, .2)}${t("fa-calendar-alt", "Khai Trương:", e.khaiTruong, .3)}${t("fa-map-marker-alt", "Maps:", `<a href="${e.maps}" target="_blank">Xem trên bản đồ</a>`, .4)}${t("fa-user-cog", "IT KV:", e.itKV, .5)}${t("fa-user-shield", "Admin:", e.admin, .6)}</div><div class="result-section"><div class="result-section-title"><i class="fas fa-tools"></i> BẢO TRÌ - KIỂM KÊ</div>${t("fa-calendar-check", "Ngày BT-KK:", e.ngayBTKK, .7)}${t("fa-file-alt", "BC Bảo Trì:", e.bcBT, .8)}${t("fa-clipboard-check", "BC Kiểm Kê:", e.bcKK, .9)}</div></div>` }
function initSearchHangBKPage() { const e = document.getElementById("searchButtonHangBK"), t = document.getElementById("clearButtonHangBK"); e && t && (e.addEventListener("click", handleSearchHangBK), t.addEventListener("click", clearSearchHangBK)) }
async function handleSearchHangBK() { const e = document.getElementById("maKhoSelect"), t = document.getElementById("maUserInput"), o = document.getElementById("searchButtonHangBK"), n = document.getElementById("buttonTextHangBK"), i = document.getElementById("resultTableContainerHangBK"), s = document.getElementById("resultTableBodyHangBK"), a = document.getElementById("errorMessageHangBK"), d = document.getElementById("noResultsMessageHangBK"), r = document.getElementById("loadingMessageHangBK"), l = e.value, c = t.value.trim(); i.style.display = "none", s.innerHTML = "", a.textContent = "", d.style.display = "none", e.classList.remove("input-error"), t.classList.remove("input-error"); let u = !1; l || (a.textContent = "Vui lòng chọn Mã Kho. ", e.classList.add("input-error"), u = !0), c || (a.textContent += "Vui lòng nhập hoặc chọn Mã Nhân Viên.", t.classList.add("input-error"), u = !0); if (u) return; o.disabled = !0, n.textContent = "Đang tìm...", r.style.display = "block"; try { const e = await callApi("searchHangBK", { maKho: l, maUser: c }); e && e.length > 0 ? (e.forEach(e => { const t = s.insertRow(); t.insertCell().textContent = e.maNV, t.insertCell().textContent = e.maTaiSan, t.insertCell().textContent = e.tenTaiSan, t.insertCell().textContent = e.loaiTaiSan, t.insertCell().textContent = e.ngayNhap, t.insertCell().textContent = e.trangThai, t.insertCell().textContent = e.maKho, t.insertCell().textContent = e.tenKho, t.insertCell().textContent = e.userVaTenSoHuu }), i.style.display = "block") : (d.style.display = "block", i.style.display = "block") } catch (e) { a.textContent = "Đã xảy ra lỗi: " + e.message } finally { o.disabled = !1, n.textContent = "Tìm Kiếm", r.style.display = "none" } }
...
function updateClock() { const e = new Date, t = document.getElementById("clock-time"), o = document.getElementById("clock-date"); t && (t.textContent = e.toLocaleTimeString("vi-VN")), o && (o.textContent = e.toLocaleDateString("vi-VN", { weekday: "long", day: "numeric", month: "long" })) }
function handleAdminLogin() { const e = document.getElementById("adminUsername"), t = document.getElementById("adminPassword"), o = document.getElementById("adminLoginError"), n = document.getElementById("adminLoginSubmit"), i = document.getElementById("adminLoginModal"), s = e.value.trim(), a = t.value; if (!s || !a) return void (o.textContent = "Vui lòng nhập đủ thông tin."); n.disabled = !0, n.textContent = "Đang kiểm tra...", o.textContent = "", setTimeout(() => { "admin" === s && "dictionary" === a || "mwang" === s && "1412" === a ? (isAdminAuthenticated = !0, i.style.display = "none", e.value = "", t.value = "", document.getElementById("btnADMIN").click()) : (isAdminAuthenticated = !1, o.textContent = "Sai tên đăng nhập hoặc mật khẩu.", t.value = "", e.focus()), n.disabled = !1, n.textContent = "Đăng nhập" }, 500) }
function handleMobileWelcomePopup() { if (!(("ontouchstart" in window || navigator.maxTouchPoints > 0) && window.innerWidth <= 1080)) return; if (sessionStorage.getItem("popupShown")) return; const e = '<div class="mobile-popup-content"><button class="mobile-popup-close">×</button><p>Đây là giao diện cho di động, chọn chế độ "xem trang web cho máy tính" trên trình duyệt, hoặc truy cập trang trên máy tính để có trải nghiệm tốt nhất.</p><div class="mobile-popup-timer">Tự động đóng sau <span class="popup-countdown">15</span> giây...</div></div>', t = document.createElement("div"); t.id = "mobile-welcome-popup", t.className = "mobile-popup", t.innerHTML = e, document.body.appendChild(t), setTimeout(() => { t && t.classList.add("show") }, 20), sessionStorage.setItem("popupShown", "true"); const o = t.querySelector(".mobile-popup-close"), n = t.querySelector(".popup-countdown"); let i = 15; const s = () => { t && (t.classList.remove("show"), setTimeout(() => { t && t.remove() }, 300)), clearInterval(a), clearTimeout(d) }; var a = setInterval(() => { --i, n && (n.textContent = i), i <= 0 && clearInterval(a) }, 1e3); var d = setTimeout(s, 15e3); o.addEventListener("click", s) }
