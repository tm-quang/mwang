...
(function() {
    ...
    if (window.location.pathname.includes('trang-chu.html')) {
        ...
        // Cập nhật giao diện chào mừng và chế độ khách
        ...
        // Hiển thị nội dung trang sau khi kiểm tra xong để tránh "chớp" màn hình
    ...
    function flattenMenu(items) {
        items.forEach(item => {
            if (item.isDropdown) {
                ...
                searchableMenuItems.push({ text: item.text, icon: item.icon, originalItem: item });
            }
        });
    }
    leftMenuData.forEach(section => { flattenMenu(section.items); });
}
...
function renderLeftMenu() {
    const leftSidebarContentWrapper = document.querySelector('#left-sidebar-container .sidebar-content-wrapper');
            ...
            if (item.isDropdown) {
                ...
                item.subItems.forEach(subItem => {
                    ...
                    subLink.addEventListener('click', (e) => {
                        ...
                        if (!isSidebarPinned || isMobileView) { closeMobileSidebar(); }
                    });
...
function renderRightMenu() {
    const rightSidebarContentWrapper = document.querySelector('#right-sidebar-container .sidebar-content-wrapper');
...
async function loadFunctionContent(item) {
    ...
    currentPageTitle.textContent = item.pageTitle || item.text || '';
    try {
        let htmlContent = '';
        if (item.htmlFile) {
        ...
        if (item.id === 'btnTimSieuThi') { initSearchStorePage(); }
        else if (item.id === 'btnTimHangBK') { initSearchHangBKPage(); }
...
async function handleSearchHangBK() {
    ...
    if (!maUser) { errorMessage.textContent += 'Vui lòng nhập hoặc chọn Mã Nhân Viên.'; maUserInput.classList.add('input-error'); hasError = true; }
    if (hasError) return;
    searchButton.disabled = true;
    buttonText.textContent = 'Đang tìm...';
    loadingMessage.style.display = 'block';
    ...
    } catch (error) { errorMessage.textContent = 'Đã xảy ra lỗi: ' + error.message; }
    finally { searchButton.disabled = false; buttonText.textContent = 'Tìm Kiếm'; loadingMessage.style.display = 'none'; }
}
