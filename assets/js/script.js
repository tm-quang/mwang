...
async function handleLogin(event) {
    ...
    try {
        ...
        if (response.success) {
            ...
            // *** NÂNG CẤP: Lưu lại trạng thái admin ***
...
function showAdminLoginModal(onSuccess) {
    ...
    const handleSubmit = async () => {
        ...
        if (response.success) {
            ...
            adminLoginError.textContent = response.message || 'Sai tên đăng nhập hoặc mật khẩu.';
        }
    };

    const handleCancel = () => {
        adminLoginSubmit.removeEventListener('click', handleSubmit);
        adminLoginCancel.removeEventListener('click', handleCancel);
        adminLoginModal.style.display = 'none';
    };

    adminLoginSubmit.addEventListener('click', handleSubmit, { once: true });
    adminLoginCancel.addEventListener('click', handleCancel, { once: true });
}
